	...
	public boolean isValid() {
		for(int quadrantxCounter = 0; quadrantxCounter <3; quadrantxCounter++) {
			for(int quadrantyCounter = 0; quadrantyCounter <3; quadrantyCounter++) {			
				if(!isQuadrantValid(quadrantxCounter, quadrantyCounter)) {return false;}
			}
		}
		for(int rowCounter = 0; rowCounter < 9; rowCounter ++) {
			if(!isRowValid(rowCounter)) {return false;}
		}
		
		for(int columnCounter = 0; columnCounter < 9; columnCounter ++) {
			if(!isColumnValid(columnCounter)) {return false;}
		}
	...
	public boolean isQuadrantValid(int quadrantX, int quadrantY) {
		...
		for(int quadrantxCounter = 0; quadrantxCounter <3; quadrantxCounter++) {
			for(int quadrantyCounter = 0; quadrantyCounter <3; quadrantyCounter++) {
				...
				if(valueFound != 0 && !set.add(valueFound)) {return false;}
			}
		}
		return true;
	}
	...
	public String toString() {
		...
		for(int rowCounter = 0; rowCounter < 9; rowCounter ++) {
			for(int columnCounter = 0; columnCounter < 9; columnCounter++) {
				...
				if(columnCounter % 3 == 2 && columnCounter < 8) {builder.append("|");}
			}
				builder.append("\n");
				if (rowCounter % 3 == 2 && rowCounter < 8) {builder.append("---|---|---\n");}
		}
